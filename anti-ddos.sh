...
# Allow outgoing SmartNode requests.
$IPTABLES -A OUTPUT -m state --state NEW -p tcp --dport 9678 -j ACCEPT
...
$IPTABLES -A OUTPUT -m state --state NEW -p tcp --dport 14855 -j ACCEPT
...
# Allow incomming SmartNode requests.
$IPTABLES -A INPUT -m state --state NEW -p tcp --dport 9678 -j ACCEPT
...
$IPTABLES -A INPUT -m state --state NEW -p tcp --dport 14855 -j ACCEPT
